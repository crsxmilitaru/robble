...
export class RackRenderer {
  ...
  updateRack(rack: Tile[], selectedIndex: number): void {
    this.rack = rack;
    this.selectedIndex = selectedIndex;
  }
  ...
  render(): void {
    ...
    this.rack.forEach((tile, idx) => {
      ...
      el.innerHTML = tile.isBlank
        ? '<span>?</span>'
        : `<span>${tile.letter}</span><span class="tile-points">${tile.points}</span>`;
