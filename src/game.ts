...
export class Game implements GameState {
  ...
  private ai: AI;
  private scorePreviewRequestId = 0;
  ...
  constructor(difficulty?: DifficultyLevel) {
    ...
    this.rackRenderer = new RackRenderer('player-tiles', this.playerRack, this.selectedTileIndex, {
  ...
  private dealInitialTiles(): void {
    for (let i = 0; i < RACK_SIZE; i++) {
      this.drawTileIntoRack(this.playerRack);
      this.drawTileIntoRack(this.computerRack);
    }
  }
  ...
  private drawTileIntoRack(rack: Tile[]): void {
    const tile = this.drawTile();
  ...
  private onCellClick(row: number, col: number): void {
    ...
    if (this.board[row][col] && !this.placedThisTurn.some((p) => p.row === row && p.col === col)) {
      return;
    }

    if (this.placedThisTurn.some((p) => p.row === row && p.col === col)) {
      this.returnTileToRack(row, col);
      return;
    }
    ...
    if (tile.isBlank) {
      promptBlankTileLetter((chosenLetter) => {
        const assignedLetter = chosenLetter.toUpperCase();
        this.playerRack.splice(this.selectedTileIndex, 1);
        this.board[row][col] = createBoardCell(tile, assignedLetter);
        this.placedThisTurn.push({ row, col, tile, assignedLetter });
    ...
    this.playerRack.splice(this.selectedTileIndex, 1);
    this.board[row][col] = createBoardCell(tile);
  ...
  recallTiles(): void {
    this.placedThisTurn.forEach((p) => {
      this.board[p.row][p.col] = null;
      this.playerRack.push(p.tile);
    });
  ...
  private validatePlacement(): ValidationResult {
    if (this.placedThisTurn.length === 0) {
      return { valid: false, error: 'Nu ai plasat nicio piesă!' };
    }
    ...
    if (!sameRow && !sameCol) {
      return { valid: false, error: 'Piesele trebuie plasate pe un rând sau o coloană!' };
    }
    ...
    if (sameRow) {
      ...
      for (let c = minC; c <= maxC; c++) {
        if (!this.board[row][c]) {
          return { valid: false, error: 'Piesele trebuie să formeze un cuvânt continuu!' };
        }
      }
    } else {
      const col = cols[0];
    ...
    if (this.firstMove) {
      if (!positions.some((p) => p.row === CENTER && p.col === CENTER)) {
        return { valid: false, error: 'Prima mutare trebuie să treacă prin centrul tablei (★)!' };
      }
    } else {
      ...
      if (!touchesExisting) {
        return { valid: false, error: 'Cuvântul trebuie să se conecteze cu piesele existente!' };
      }
    }
  ...
  async submitMove(): Promise<void> {
    ...
    if (words.length === 0) {
      this.setMessage('Nu s-a format niciun cuvânt!');
      return;
    }
    ...
    this.history.push({
      player: 'player',
      words: words.map((w) => ({ word: w.word.toUpperCase(), score: w.score })),
    ...
    this.refillRack(this.playerRack);
    this.selectedTileIndex = -1;
  ...
  private async computerTurn(): Promise<void> {
    ...
    if (result) {
      result.placements.forEach((p) => {
        this.board[p.row][p.col] = createBoardCell(p.tile, p.assignedLetter);
        const idx = this.computerRack.findIndex((t) => t.id === p.tile.id);

        if (idx !== -1) {
          this.computerRack.splice(idx, 1);
        }
      });
      ...
      this.history.push({
        player: 'computer',
        words: result.words.map((w) => ({ word: w.toUpperCase(), score: 0 })),
        totalScore: result.score,
        isRobble: result.placements.length === RACK_SIZE
      });
  ...
  private endGame(): void {
    this.gameOver = true;
    const playerPenalty = this.playerRack.reduce((sum, t) => sum + t.points, 0);
    const computerPenalty = this.computerRack.reduce((sum, t) => sum + t.points, 0);

    this.playerScore -= playerPenalty;
    this.computerScore -= computerPenalty;

    if (this.playerRack.length === 0) {
      this.playerScore += computerPenalty;
    }

    if (this.computerRack.length === 0) {
      this.computerScore += playerPenalty;
    }
    ...
    this.showModal(
      ...
      () => this.newGame(),
      () => {
        this.setMessage('Jocul s-a terminat.');
      },
      false,
      'Joc nou',
      'Vezi tabla'
  ...
  newGame(newDifficulty?: DifficultyLevel): void {
    ...
    this.setMessage('Joc nou! Plasează piesele pe tablă.');
    updateSubmitButton(false);
    this.hideScorePreview();
  }
  ...
  exchangeDialog(): void {
    ...
    if (this.bag.length < RACK_SIZE) {
      this.setMessage(`Nu poți schimba piese dacă în sac sunt mai puțin de ${RACK_SIZE} piese!`);
      return;
    }
  ...
  private performExchange(indices: number[]): void {
    ...
    for (const idx of indices) {
      toExchange.push(this.playerRack.splice(idx, 1)[0]);
    }

    this.bag.push(...toExchange);
    this.shuffle(this.bag);
    this.refillRack(this.playerRack);

    this.setMessage(`Ai schimbat ${indices.length} piese.`);
    this.isPlayerTurn = false;
    this.consecutivePasses++;
    this.saveGame();
    this.render();
  ...
  private updateScores(): void {
    updateScoreDisplay(this.playerScore, this.computerScore, this.isPlayerTurn);
  }
  ...
  private async updateScorePreview(): Promise<void> {
    ...
    const requestId = ++this.scorePreviewRequestId;
    const validation = this.validatePlacement();
    ...
    try {
      ...
      if (requestId !== this.scorePreviewRequestId) {
        return;
      }
